...
var rule = {
    ...
    searchUrl:'https://search.bilibili.com/all?keyword=**&search_source=1',
    searchable:1,
    filterable:1,
    quickSearch:0,
    headers:{
    ...
    // 核心修改：完善弹幕功能的lazy函数
    lazy: $js.toString(() => {
        // 弹幕接口配置（兼容B站原生弹幕和自定义弹幕接口）
        const DANMU_API = "http://43.248.192.34:666/dm/dm.php?url=";
        ...
        try {
            if (/^http/.test(input)) {
                ...
                input = {
                    ...
                    // 添加弹幕参数（双保险：B站原生弹幕 + 自定义弹幕接口）
                    danmaku: DANMU_API + videoUrl
                };
            } else {
                // 处理eid_cid格式的参数
                ...
                if (ids.length === 2) {
                    ...
                    // B站原生弹幕接口
                    let biliDanmu = 'https://api.bilibili.com/x/v1/dm/list.so?oid=' + cid;
                    // 自定义弹幕接口
                    let customDanmu = DANMU_API + cid;
                    
                    let url = "https://api.bilibili.com/pgc/player/web/playurl?qn=116&ep_id=" + eid + "&cid=" + cid;
                    ...
                    input = {
                        ...
                        url: input,
                        parse: 0,
                        danmaku: DANMU_API + input
                    };
                }
            }
        } catch (e) {
            // 异常处理：确保播放功能不受影响
            log("弹幕加载异常：" + e.message);
            input = {
                jx: 1,
                url: input,
                parse: 0,
