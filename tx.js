var rule = {
    ...
    class_name: 'Q群:1091711512&电影4K&电视剧4K&综艺4K&动漫4K&少儿4K&纪录片4K',
        ...
        try {
            ...
            if (bata.url.includes("qq")) {
                input = {
                    ...
                    url: bata.url,
                    jx: 1,
                    danmaku: DANMU_API + input.split("?")[0]
                };
            } else {
                input = {
                    parse: 1,
    ...
    二级: $js.toString(() => {
        ...
        // ========== 弹幕内嵌（使用 ||danmu= 格式）==========
        function attachDanmu(url) {
            if (url.includes('||danmu=')) return url;
            return url;
        }
        zp.forEach(item => { item.url = attachDanmu(item.url); });
        yg.forEach(item => { item.url = attachDanmu(item.url); });
        // =================================================

        VOD.vod_play_from = yg.length < 1 ? "qq" : "qq$$$qq 预告及花絮";
        VOD.vod_play_url = yg.length < 1 ? zp.map(function (it) {
